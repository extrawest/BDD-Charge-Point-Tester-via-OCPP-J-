...
public class ChangeConfigurationConfirmationBddHandler extends IncomingMessageFieldsAssertionFactory<ChangeConfigurationConfirmation>
        implements IncomingMessageFactory<ChangeConfigurationConfirmation> {
        ...
        this.requiredFieldsSetup = Map.of(
                STATUS_REQUIRED, (conf, status) -> {
                    if (nonEqual(wildCard, status)) {
                        conf.setStatus(getValidatedEnumValueOrThrow(ConfigurationStatus.class, status, STATUS_REQUIRED));
                    }
                }
        );
    ...
    @Override
    public void validateAndAssertFieldsWithParams(Map<String, String> params, ChangeConfigurationConfirmation message) {
