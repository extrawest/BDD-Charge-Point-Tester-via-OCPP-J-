...
public class ClearChargingProfileConfirmationBddHandler extends IncomingMessageFieldsAssertionFactory<ClearChargingProfileConfirmation>
        ...
        this.requiredFieldsSetup = Map.of(
                STATUS_REQUIRED, (conf, status) -> {
                    if (nonEqual(wildCard, status)) {
                        conf.setStatus(getValidatedEnumValueOrThrow(ClearChargingProfileStatus.class, status, STATUS_REQUIRED));
                    }
                }
        );
    ...
    @Override
    public void validateAndAssertFieldsWithParams(Map<String, String> params, ClearChargingProfileConfirmation message) {
