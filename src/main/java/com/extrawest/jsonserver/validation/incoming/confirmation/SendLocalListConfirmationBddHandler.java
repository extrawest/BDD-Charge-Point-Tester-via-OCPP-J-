...
public class SendLocalListConfirmationBddHandler extends IncomingMessageFieldsAssertionFactory<SendLocalListConfirmation>
        implements IncomingMessageFactory<SendLocalListConfirmation> {
        ...
        this.requiredFieldsSetup = Map.of(
                STATUS_REQUIRED, (conf, status) -> {
                    if (nonEqual(wildCard, status)) {
                        conf.setStatus(getValidatedEnumValueOrThrow(UpdateStatus.class, status, STATUS_REQUIRED));
                    }
                }
        );
    ...
    @Override
    public void validateAndAssertFieldsWithParams(Map<String, String> params, SendLocalListConfirmation message) {
